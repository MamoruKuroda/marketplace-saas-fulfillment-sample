...
並べません。結果の後の **提供元の裏側を見る / See behind the partner site** は任意の記録確認です。
実際の保存状態と、この契約の `/admin/{guid}` への直接リンクを表示します。役割切替を必須にはしません。

実装の説明、用語集、関連コード、職種別の見どころは、画面内ではなく
[walkthrough](docs/walkthrough.ja.md) にまとめています。各画面の下部に小さな **実装ガイド ↗**
リンクを置き、日本語なら日本語版、英語なら英語版のリポジトリ文書を開きます。
購入トークン・契約IDなどのクエリ情報は渡しません。サーバーAPI・認証・課金・状態遷移・DBスキーマは変更しません。
...
全体図と実際の操作・保存結果はデモに残し、詳しい学習はリポジトリで行います。
以前の `#how` ブックマークからは実装ガイドのリンクに移動し、自動で別サイトへは転送しません。
...
- リポジトリへの参照や保存記録へのリンクは、模擬ロールの変更やアクセス権付与ではありません。
