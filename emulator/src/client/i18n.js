(function () {
    const I18N = {
        en: {
            ...
            "offerTile.year": "year",
            "index.infoHtml": "<p><strong>This emulator stands in for Microsoft.</strong> Use it to drive a purchase and the whole subscription lifecycle without a real Marketplace transaction — you play all three roles.</p><ol class=\"steps\"><li><strong>Here (Marketplace) — you are the buyer:</strong> pick an offer with \"Get it now\", choose a plan, then <strong>Continue</strong>. (Microsoft runs the store and billing.) This hands off to the publisher's landing page with a purchase token.</li><li><strong>Landing page — you are the buyer:</strong> the publisher's app resolves the token and you confirm <strong>Activate</strong>. The subscription becomes Subscribed and billing starts.</li><li><strong>Publisher admin — you are the publisher:</strong> the publisher's app shows the authoritative subscription state (its source of truth).</li><li><strong>Back here (Subscriptions tab) — Microsoft's side:</strong> trigger lifecycle webhooks — change plan/quantity, suspend/reinstate, unsubscribe — then refresh the admin to watch the state follow.</li></ol><p>The emulator ships with two sample offers (<strong>flat-rate</strong> and <strong>per-user</strong>), each with sample plans. API reference: <a href=\"https://learn.microsoft.com/azure/marketplace/partner-center-portal/pc-saas-fulfillment-subscription-api\" target=\"_blank\" rel=\"noopener\">SaaS fulfillment Subscription APIs v2</a>.</p>",
        ...
        ja: {
            ...
            "offerTile.month": "月",
            "offerTile.year": "年",
            "index.infoHtml": "<p><strong>このエミュレーターは Microsoft 役です。</strong> 実際のマーケットプレース取引なしに、購入とサブスクリプションのライフサイクル全体を動かせます。あなたが3つの役割すべてを演じます。</p><ol class=\"steps\"><li><strong>ここ（マーケットプレース）＝あなたは購入者:</strong>「今すぐ入手」でオファーを選び、プランを選んで<strong>続行</strong>します。（ストアと課金は Microsoft が運営します。）購入トークンを付けてパブリッシャーのランディングページへ引き渡します。</li><li><strong>ランディングページ＝あなたは購入者:</strong> パブリッシャーのアプリがトークンを解決し、あなたが<strong>有効化</strong>を確認します。サブスクリプションが Subscribed になり課金が始まります。</li><li><strong>パブリッシャー管理＝あなたはパブリッシャー:</strong> パブリッシャーのアプリが権威あるサブスクリプション状態（信頼できる情報源）を表示します。</li><li><strong>ここに戻る（Subscriptions タブ）＝Microsoft 側:</strong> ライフサイクルの Webhook（プラン/数量の変更・一時停止/再開・解約）を発生させ、管理画面を再読み込みして状態が追随する様子を確認します。</li></ol><p>エミュレーターには2つのサンプルオファー（<strong>flat-rate</strong> と <strong>per-user</strong>）があり、それぞれサンプルプランを含みます。API リファレンス: <a href=\"https://learn.microsoft.com/azure/marketplace/partner-center-portal/pc-saas-fulfillment-subscription-api\" target=\"_blank\" rel=\"noopener\">SaaS fulfillment Subscription APIs v2</a>。</p>",
