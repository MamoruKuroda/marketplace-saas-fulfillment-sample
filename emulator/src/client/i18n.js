(function () {
    const I18N = {
        en: {
            ...
            "nav.offers": "Offers",
            "nav.config": "Config",
            "hint.marketplace": "Simulate a purchase as Microsoft, then hand off to the publisher's landing page.",
            "hint.subscriptions": "Drive lifecycle webhooks — change, suspend, reinstate, or cancel — on existing subscriptions.",
            "hint.landing": "Preview the landing page the emulator redirects to after a purchase.",
            "hint.offers": "Create and manage the sample offers and plans.",
            ...
            "offerTile.year": "year",
            "index.infoHtml": "<p><strong>This emulator stands in for Microsoft.</strong> Use it to drive a purchase and the whole subscription lifecycle without a real Marketplace transaction — you play all three roles.</p><ol class=\"steps\"><li><strong>Here (Marketplace) — you are Microsoft:</strong> pick an offer with \"Get it now\", choose a plan, then <strong>Continue</strong>. This hands off to the publisher's landing page with a purchase token.</li><li><strong>Landing page — you are the buyer:</strong> the publisher's app resolves the token and you confirm <strong>Activate</strong>. The subscription becomes Subscribed and billing starts.</li><li><strong>Publisher admin — you are the publisher:</strong> the publisher's app shows the authoritative subscription state (its source of truth).</li><li><strong>Back here (Subscriptions tab):</strong> trigger lifecycle webhooks — change plan/quantity, suspend/reinstate, unsubscribe — then refresh the admin to watch the state follow.</li></ol><p>The emulator ships with two sample offers (<strong>flat-rate</strong> and <strong>per-user</strong>), each with sample plans. API reference: <a href=\"https://learn.microsoft.com/azure/marketplace/partner-center-portal/pc-saas-fulfillment-subscription-api\" target=\"_blank\" rel=\"noopener\">SaaS fulfillment Subscription APIs v2</a>.</p>",
        ...
        ja: {
            ...
            "nav.offers": "オファー",
            "nav.config": "設定",
            "hint.marketplace": "Microsoft 役として購入を模擬し、パブリッシャーのランディングページへ引き渡します。",
            "hint.subscriptions": "既存のサブスクリプションにライフサイクル Webhook（変更・一時停止・再開・解約）を発生させます。",
            "hint.landing": "購入後にエミュレーターがリダイレクトするランディングページのプレビューです。",
            "hint.offers": "サンプルのオファーとプランを作成・管理します。",
            ...
            "offerTile.month": "月",
            "offerTile.year": "年",
            "index.infoHtml": "<p><strong>このエミュレーターは Microsoft 役です。</strong> 実際のマーケットプレース取引なしに、購入とサブスクリプションのライフサイクル全体を動かせます。あなたが3つの役割すべてを演じます。</p><ol class=\"steps\"><li><strong>ここ（マーケットプレース）＝あなたは Microsoft:</strong>「今すぐ入手」でオファーを選び、プランを選んで<strong>続行</strong>します。購入トークンを付けてパブリッシャーのランディングページへ引き渡します。</li><li><strong>ランディングページ＝あなたは購入者:</strong> パブリッシャーのアプリがトークンを解決し、あなたが<strong>有効化</strong>を確認します。サブスクリプションが Subscribed になり課金が始まります。</li><li><strong>パブリッシャー管理＝あなたはパブリッシャー:</strong> パブリッシャーのアプリが権威あるサブスクリプション状態（信頼できる情報源）を表示します。</li><li><strong>ここに戻る（Subscriptions タブ）:</strong> ライフサイクルの Webhook（プラン/数量の変更・一時停止/再開・解約）を発生させ、管理画面を再読み込みして状態が追随する様子を確認します。</li></ol><p>エミュレーターには2つのサンプルオファー（<strong>flat-rate</strong> と <strong>per-user</strong>）があり、それぞれサンプルプランを含みます。API リファレンス: <a href=\"https://learn.microsoft.com/azure/marketplace/partner-center-portal/pc-saas-fulfillment-subscription-api\" target=\"_blank\" rel=\"noopener\">SaaS fulfillment Subscription APIs v2</a>。</p>",
